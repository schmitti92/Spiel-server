...
/** ---------- Action-Mode (Joker) state (SERVER is chef) ----------
 *  Schritt J1: NUR State mitschicken (UI kann anzeigen), noch keine Regeln ändern.
 *  -> Classic bleibt 100% unverändert.
 */
function makeActionState(activeColors){
  const all = ["red","blue","green","yellow"];
  const actSet = new Set((activeColors||[]).map(String));
  const jokersByColor = {};
  for(const c of all){
    // only active colors get usable jokers; inactive colors are false (UI kann trotzdem anzeigen)
    const enabled = actSet.has(c);
    jokersByColor[c] = {
      choose: !!enabled,
      sum: !!enabled,
      allColors: !!enabled,
  ...
  return {
    jokersByColor,
    effects: {
      // später: { allColorsBy, doubleRoll:{kind,by,rolls}, barricadeBy }
    }
  };
}
...
function initGameState(room, activeColors, mode="classic") {
  ...
  // mode is additive (classic default). Important: does NOT change rules in J1.
  room.state.mode = (mode === "action") ? "action" : "classic";
  room.state.action = (room.state.mode === "action") ? makeActionState(activeColors) : null;
...
room.players.set(clientId, { id: clientId, name, color, isHost, sessionToken, lastSeen: Date.now() });
    ...
    if (msg.type === "start") {
      ...
      initGameState(room, uniqueAct, (msg.mode === "action" ? "action" : "classic"));
      await persistRoomState(room);
