...
wss.on("connection", (ws) => {
  ...
  ws.on("message", async (buf) => {
    ...
    if (msg.type === "join") {
      ...
      const forceNew = !!msg.forceNew; // host-only: ignore old save + start fresh
      ...
      // If host explicitly wants a fresh game, wipe persisted state and SKIP restore.
      if (asHost && forceNew) {
        await deletePersisted(room);
        room.state = null;
        room.lastRollWasSix = false;
        room.carryingByColor = { red: false, blue: false, green: false, yellow: false };
        console.log(`[host] forceNew -> wiped persisted state for room=${roomCode}`);
      } else {
