...
(() => {
  ...
  // ===== Legendary Dice Dock (VISUAL ONLY) =====
  // Keine Spiellogik wird geändert – wir verschieben nur das DOM-Element #diceCube in die Status-Card
  // und skalieren es groß. Fail-safe: wenn Struktur nicht gefunden wird, bleibt alles wie es ist.
  function dockDiceIntoStatusCard(){
    try{
      if(!diceEl) return;

      // already docked?
      if(diceEl.dataset && diceEl.dataset.docked === "1") return;

      const anchor = boardInfo || barrInfo;
      if(!anchor) return;

      // Find the nearest "Status card" container that contains both board & barricade infos.
      let card = anchor.closest("section,article,div");
      for(let i=0;i<8 && card;i++){
        const hasBoard = !!card.querySelector("#boardInfo");
        const hasBarr  = !!card.querySelector("#barrInfo");
        if(hasBoard && hasBarr) break;
        card = card.parentElement;
      }
      if(!card) return;

      // Create a wrapper so scaling doesn't break the 3D cube
      let wrap = card.querySelector("#diceDockWrap");
      if(!wrap){
      ...
      // Scale big (you can tweak this value)
      wrap.style.transform = "scale(2.6)";
      wrap.style.transformOrigin = "top right";

      // Insert right BELOW the Status title if possible (so it's directly over Board/Barikaden)
      const titleEl = Array.from(card.querySelectorAll("h1,h2,h3,h4,div,span"))
        .find(el => (el.textContent || "").trim() === "Status");

      if(titleEl && titleEl.parentElement){
        // place after title element
        if(titleEl.nextSibling){
          titleEl.parentElement.insertBefore(wrap, titleEl.nextSibling);
        }else{
          titleEl.parentElement.appendChild(wrap);
        }
      }else{
        // fallback: place at top of card
        card.insertBefore(wrap, card.firstChild);
      }

      // Move the actual dice element into wrapper
      wrap.appendChild(diceEl);
      if(diceEl.dataset) diceEl.dataset.docked = "1";

      // Make sure it stays clickable
      diceEl.style.pointerEvents = "auto";
      diceEl.style.touchAction = "manipulation";
    }catch(_e){}
  }

  // Try docking after DOM is ready and whenever layout changes
  try{
    if(document.readyState === "loading"){
      document.addEventListener("DOMContentLoaded", () => {
        dockDiceIntoStatusCard();
        setTimeout(dockDiceIntoStatusCard, 200);
        setTimeout(dockDiceIntoStatusCard, 800);
      });
    }else{
      dockDiceIntoStatusCard();
      setTimeout(dockDiceIntoStatusCard, 200);
    ...
    const mo = new MutationObserver(() => dockDiceIntoStatusCard());
