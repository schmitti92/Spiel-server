...
function initGameState(room) {
  ...
  // Active (spielbar) colors are only the colors of CONNECTED players who picked a color.
  // This keeps 2–4 players possible, while ALL 4 colors remain visible in the houses.
  const activeColors = Array.from(room.players.values())
    .filter(p => p.color && isConnectedPlayer(p) && ALL_COLORS.includes(p.color))
    .map(p => p.color);

  // fallback safety
  const uniq = [];
  for(const c of activeColors){ if(!uniq.includes(c)) uniq.push(c); }
  const playersTurn = (uniq.length >= 2) ? uniq : ["red","blue"];

  // pieces 5 per color in house (ALL colors for visibility)
  const pieces = [];
  ...
  // choose starter among ACTIVE colors
  const turnColor = playersTurn[Math.floor(Math.random() * playersTurn.length)];
  ...
  room.state = {
    started: true,
    paused: false,
    players: playersTurn,        // turn order list (2–4)
    turnColor,
...
function nextTurnColor(room, current){
  const order = Array.isArray(room?.state?.players) ? room.state.players : ["red","blue"];
  const idx = order.indexOf(current);
  return order[(idx + 1) % order.length] || order[0] || "red";
}
